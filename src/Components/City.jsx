...
const City = ({ isLoading, forecastData, cityName }) => {
  ...
  const fetchWeatherForcast = async (selectedCity) => {
    try {
      const forcastResponseData = await fetch(
        `https://api.openweathermap.org/data/2.5/forecast?q=${selectedCity.label}&APPID=d37deb3cd8797e9402bfeeb15ddc404a`
      );
      ...
      const currentWeatherResponseData = await fetch(
        `https://api.openweathermap.org/data/2.5/weather?q=${selectedCity.label}&APPID=d37deb3cd8797e9402bfeeb15ddc404a`
      );
